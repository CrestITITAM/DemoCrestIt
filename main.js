            ...
            response.on('data', (chunk) => {
              ...
              if (chunk) {
                ...
                //  console.log('get_company_data'+obj.system_asset_id);
                 // logEverywhere(obj);
                  if(obj.status == 'valid'){
                    e.reply('checked_company_logo', obj.result,server_url,obj.System_Key,global.assetID);
                  }else if(obj.status == 'invalid'){
                    e.reply('checked_company_logo', obj.result,server_url,obj.System_Key,global.assetID);
                  }
                } catch (e) {
...
ipcMain.on('checkinstalledlicenses', function(e, form_data) {
 ...
 // logEverywhere('In check Call');
  ...
  request.on('error', (error) => {
      console.error('Error while checking install and allocated licenses', error);
  });
